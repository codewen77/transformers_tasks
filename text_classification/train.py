...
from text_classification.preprocess import DataProcessor
...
def evaluate_model(model, metric, dev_iterator):
    """
    在测试集上评估当前模型的训练效果。
    ...
    Args:
        model: 当前模型
        metric: 评估指标类(metric)
        dev_iterator: 测试集的dataloader
    """
    model.eval()
    with torch.no_grad():
        for step, (batch, labels) in enumerate(dev_iterator):
            outputs = model(input_ids=batch['input_ids'],
                            token_type_ids=batch['token_type_ids'],
                            attention_mask=batch['attention_mask'])
            predictions = outputs.logits.argmax(dim=-1).detach().cpu().numpy().tolist()
            metric.add_batch(pred_batch=predictions, gold_batch=labels.detach().cpu().numpy().tolist())
    eval_metric = metric.compute(4)
    model.train()
...
def train():
    ...
    train_iterator = DataProcessor(args.train_path, args.device, tokenizer, args.batch_size, args.max_seq_len,
                                   args.seed)
    dev_iterator = DataProcessor(args.dev_path, args.device, tokenizer, args.batch_size, args.max_seq_len, args.seed)
    ...
    num_update_steps_per_epoch = len(train_iterator)
    max_train_steps = args.num_train_epochs * num_update_steps_per_epoch
    warm_steps = int(args.warmup_ratio * max_train_steps)
    lr_scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=warm_steps,
    ...
    for epoch in range(1, args.num_train_epochs + 1):
        for batch, labels in train_iterator:
            ...
            if global_step % args.valid_steps == 0:
                ...
                acc, precision, recall, f1, class_metrics = evaluate_model(model, metric, dev_iterator)
